...
public final class Bukkit {
	...
	public static void setServer(Server server) {
		...
		server.getLogger()
		      .info("This server is running " + getName() + " version " + getVersion() + " (Implementing API version " +
		            getBukkitVersion() + ")");
	}
	 ...
	 * <p>
	 * This method exists for legacy reasons to provide backwards compatibility. It will not exist at runtime and should
	...
	/**
	 * Gets a view of all currently logged in players. This {@linkplain Collections#unmodifiableCollection(Collection)
	 * view} is a reused object, making some operations like {@link Collection#size()} zero-allocation.
	 * <p>
	 * The collection is a view backed by the internal representation, such that, changes to the internal state of the
	 * server will be reflected immediately. However, the reuse of the returned collection (identity) is not strictly
	 * guaranteed for future or all implementations. Casting the collection, or relying on interface implementations
	 * (like {@link Serializable} or {@link List}), is deprecated.
	 * <p>
	 * Iteration behavior is undefined outside of self-contained main-thread uses. Normal and immediate iterator use
	 * without consequences that affect the collection are fully supported. The effects following (non-exhaustive)
	 * {@link Entity#teleport(Location) teleportation}, {@link Player#setHealth(double) death}, and {@link
	 * Player#kickPlayer(String) kicking} are undefined. Any use of this collection from asynchronous threads is
	 * unsafe.
	 * <p>
	 * For safe consequential iteration or mimicking the old array behavior, using {@link Collection#toArray(Object[])}
	 * is recommended. For making snapshots, {@link ImmutableList#copyOf(Collection)} is recommended.
	 *
	 ...
	 * <p>
	 * This is the same as calling {@link #broadcast(java.lang.String, java.lang.String)} to {@link
	 ...
	 * <p>
	 * <b>Example Usage:</b> <ul> <li>A value of 1 will mean the server will attempt to spawn monsters every tick. <li>A
	 * value of 400 will mean the server will attempt to spawn monsters every 400th tick. <li>A value below 0 will be
	 * reset back to Minecraft's default. </ul>
	 * <p>
	 * <b>Note:</b> If set to 0, animal spawning will be disabled. We recommend using spawn-animals to control this
	 * instead.
	 * <p>
	 ...
	 * reset back to Minecraft's default. </ul>
	 * <p>
	 * <b>Note:</b> If set to 0, monsters spawning will be disabled. We recommend using spawn-monsters to control this
	 * instead.
	 * <p>
	 * Minecraft default: 1.
	 *
	 ...
	 * <p>
	 * This list is not sorted in any particular order. If an exact match is found, the returned list will only contain
	 * a single result.
	 *
	...
	/**
	 * Get a list of all recipes for a given item. The stack size is ignored in comparisons. If the durability is -1, it
	 ...
	 * <p>
	 * <ul> <li>Vanilla behaviour: check for collisions and move the player if needed. <li>Exact behaviour: spawn
	 * players exactly where they should be. </ul>
	 *
	 * @return true if exact location locations are used for spawning, false for vanilla collision detection or
	 * otherwise
	 *
	 ...
	 * <p>
	 * Bans by name are no longer supported and this method will return null when trying to request them. The
	 * replacement is bans by UUID.
	 *
	...
	/**
	 * Creates an empty inventory of the specified type. If the type is {@link InventoryType#CHEST}, the new inventory
	 * has a size of 27; otherwise the new inventory has the normal size for its type.
	 *
	...
	/**
	 * Creates an empty inventory with the specified type and title. If the type is {@link InventoryType#CHEST}, the new
	 * inventory has a size of 27; otherwise the new inventory has the normal size for its type.<br> It should be noted
	 * that some inventory types do not support titles and may not render with said titles on the Minecraft client.
	 *
	 ...
	 * thread matching the main thread indicates that it is synchronized, but a mismatch <b>does not preclude</b> the
	 * same assumption.
	 *
	 ...
	 *
	 * @return the default server-icon; null values may be used by the implementation to indicate no defined icon, but
	 ...
	 * <p>
	 * See {@link ChunkGenerator#generateChunkData(org.bukkit.World, java.util.Random, int, int,
